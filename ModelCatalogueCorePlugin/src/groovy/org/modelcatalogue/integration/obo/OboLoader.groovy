        ...
        builder.build {
            classification(name: name) {
                ...
                document.termFrames.eachWithIndex { Frame frame, i ->
                    ...
                    String log1 =  "${(i + 1).toString().padLeft(6, '0')}"
                    String log2 =  "${document.termFrames.size().toString().padLeft(6, '0')}"
                    String log3 =  "Importing model ${frame.id}: ${frame.getClause('name')?.value}"
                    log.info "Frame: " + log1 + log2 + log3
